                          ...
                          HICON hIcon, HMENU hMenu, int x, int y )
{
    ...
    if( DXUTGetHWNDFocus() == NULL )
    {
        ...
        // be resized later
		DWORD dwWindowStyle = WS_POPUP | WS_CAPTION | WS_SYSMENU | 
                              WS_MINIMIZEBOX;
...
HRESULT DXUTToggleFullScreen()
{
	...
	int nWidth = deviceSettings.pp.BackBufferWidth; //rcWindowClient.right - rcWindowClient.left;
    int nHeight = deviceSettings.pp.BackBufferHeight; //rcWindowClient.bottom - rcWindowClient.top;
    ...
    if( SUCCEEDED(hr) ) 
    {
        ...
        if( FAILED(hr) )
        {
            ...
            nWidth = deviceSettings.pp.BackBufferWidth; //rcWindowClient.right - rcWindowClient.left;
            nHeight = deviceSettings.pp.BackBufferHeight; //rcWindowClient.bottom - rcWindowClient.top;
            if( nWidth > 0 && nHeight > 0 )
            {
                matchOptions.eResolution = DXUTMT_CLOSEST_TO_INPUT;
                deviceSettings.pp.BackBufferWidth = nWidth;
...
HRESULT DXUTChangeDevice( DXUTDeviceSettings* pNewDeviceSettings, IDirect3DDevice9* pd3dDeviceFromApp, bool bForceRecreate )
{
	...
	//HACK
	//pNewDeviceSettings->pp.MultiSampleType = D3DMULTISAMPLE_NONMASKABLE;
	//pNewDeviceSettings->pp.MultiSampleQuality = 0;
	//HACK
