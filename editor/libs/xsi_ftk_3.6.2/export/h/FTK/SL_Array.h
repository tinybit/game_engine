...
)
{
    ...
    if (( m_lNbAllocElem - m_lNbUsedElem ) > i_lMaxWasted )
    {
		...
		CElemType * l_pNewArray = (CElemType *) new CSubElemType[m_lNbUsedElem * StructSize];
        ...
        if ( l_pNewArray != NULL )
        {
			...
			for ( register SI_Long i = 0; i < GetUsed(); i++)
			{
				l_pNewArray[i] = l_pCurrentArray[i];
			}

			//Delete original array
			delete [] (CSubElemType *) l_pCurrentArray;
...
()
{
	delete [] (CSubElemType *) m_pVariant->p_voidVal;
...
)
{
	...
	else
	{
		...
		// allocate new array
		CElemType * l_pNewArray = (CElemType *) new CSubElemType[l_lNewArraySize * StructSize];
