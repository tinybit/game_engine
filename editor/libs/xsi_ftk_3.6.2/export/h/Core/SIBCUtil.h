...
#define SI_Long		long				/*!< SI_Long is defines as 'long'. */
#define SI_Void		void				/*!< SI_Void is defines as 'void'. */
#define SI_ULong	unsigned long		/*!< SI_ULong is defines as 'unsigned long'. */
...
#define __SI_FGETC( fp ) si_getc( fp )
...
public:
	...
	size_t read( void *pbuffer, size_t size, size_t count )
	{
		...
		//if( false )
		{
			...
			while( count>0 )
			{
				if( remainingBytesInBuffer()<size ) fillBuffer();
