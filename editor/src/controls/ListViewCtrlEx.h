...
class CListBase
{
	public:
		...
		int GetColumnCount() const { return static_cast<int>( m_aColumnData.GetSize() ); };
		...
		virtual const CString GetToolTip( int /*nItem*/, int /*nSubItem*/, UINT /*nFlags*/, BOOL& /*bAsLabel*/ ) { return _T(""); };

		BOOL IsColumnEnabled( int nColumn ) const { return m_aColumnData[nColumn]->m_bEnabled; }
	...
	private:
		...
		struct COLUMN_DATA
		{
			COLUMN_DATA(): m_bEnabled(true), m_bHidingAllowed(false), m_bVisible(true), m_nWidth(0), m_nOrder(0), m_pLVColumn(0){};
			~COLUMN_DATA();
...
class CListCtrlEx: public CListCtrl, public CListBase
{
	...
	public:
		CListCtrlEx()// : m_subitem(0), m_bEdited(false)//, m_editWnd(NULL)
		{ 
			m_pListCtrl = reinterpret_cast<CListCtrlHelper*>( this );
			//m_editWnd.SetParent( this );
		};
		...
		virtual void DrawItem( LPDRAWITEMSTRUCT lpDrawItemStruct ) { CListBase::DrawItem( lpDrawItemStruct ); };
...
class CListViewEx: public CListView, public CListBase
{
	...
	public:
		CListViewEx() { m_pListCtrl = reinterpret_cast<CListCtrlHelper*>( &GetListCtrl() ); };
		virtual void DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) { CListBase::DrawItem(lpDrawItemStruct); };
